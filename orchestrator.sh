...
show_menu() {
    ...
    echo "  4) Parse Zone File (hostnames.zone → parsed/)"
    echo "  5) Generate infrastructure.json (Master File)"
...
while true; do
    ...
    case $choice in
        ...
        3)
            echo ""
            source "$MODULES_DIR/dkim_generator.sh"
            ;;
        4)
            echo ""
            source "$MODULES_DIR/zone_parser.sh"
            ;;
        5)
            echo ""
            echo "Generating Infrastructure.json..."
            source "$MODULES_DIR/infrastructure_generator.sh"
            ;;
        6|7|8)
