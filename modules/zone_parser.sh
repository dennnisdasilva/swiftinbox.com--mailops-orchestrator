...
# Calculate expected records from cidrs.txt
expected_records=0
if [[ -f "$PROJECT_ROOT/existing_scripts/generate-enhanced/cidrs.txt" ]]; then
    while IFS=, read -r cidr rest; do
        [[ -z "$cidr" || "${cidr:0:1}" == "#" ]] && continue
        mask="${cidr##*/}"
        case $mask in
            24) expected_records=$((expected_records + 254)) ;;
            25) expected_records=$((expected_records + 126)) ;;
            26) expected_records=$((expected_records + 62)) ;;
            27) expected_records=$((expected_records + 30)) ;;
            28) expected_records=$((expected_records + 14)) ;;
            29) expected_records=$((expected_records + 6)) ;;
            30|31) expected_records=$((expected_records + 2)) ;;
            32) expected_records=$((expected_records + 1)) ;;
        esac
    done < "$PROJECT_ROOT/existing_scripts/generate-enhanced/cidrs.txt"
fi

if [[ $expected_records -ne $record_count ]]; then
    log_error "Warning: Expected $expected_records records but parsed $record_count"
    return 1
else
    log_success "Successfully parsed all $record_count expected records"
fi
fi
